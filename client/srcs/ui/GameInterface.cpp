...
GameInterface::GameInterface ( int number_of_player ) : UIInterface("unknow")
{
	...
	// // 	img.transform.position.x = (i * 100) + (BombermanClient::getInstance()->screen->width - 510);
	// // 	Text t1 = Text("0");
	// // 	t1.transform.position.x = (i * 100) + (BombermanClient::getInstance()->screen->width- 510) + 53;
	// // 	t1.transform.position.y = 8;
	// // 	this->canvas->addText(t1);
	// // 	this->canvas->addImage(img);
	// // }
	// Image *img = new Image("assets/ui/gameinterface/clock_over.png");
	// img->transform.position.x = (BombermanClient::getInstance()->screen->width / 2) - 70;
	// img->transform.position.y = 5;
	// this->canvas->addImage("clock_ground", img);
    //
	// Text *timer = new Text("1 : 43");
	// timer->transform.position.x = (BombermanClient::getInstance()->screen->width / 2) - 50;
